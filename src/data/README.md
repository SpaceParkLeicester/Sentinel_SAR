...
* Click on the `+ Create Service Account` and give necessary details. Make sure to Copy the `Service account email`. In the Keys section, create a new key and download the key in `JSON` to your work system. Path to the key is, and the `Service email account` is needed for the authentication process, which you can find in `/Oil-Storage-Tanks/src/data/meta_data.py`